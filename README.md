...
Sapaan seperti `Hi`/`Halo` menampilkan form laporan yang harus di-copy, diisi, lalu dikirim sebagai satu pesan. Jawaban teks satu-satu ditolak sebelum form pertama diterima; setelah itu jawaban singkat diperbolehkan untuk menjawab field tertentu yang confidence-nya rendah. Foto serta Share Location biasa dari WhatsApp (bukan Live Location) tetap boleh dikirim terpisah. Untuk lokasi manual, form wajib memuat Desa/Kelurahan, Kecamatan, dan Kota/Kabupaten—lokasi ambigu seperti `Sayung, Demak` belum cukup; jika memakai Share Location, tiga baris tersebut dihapus. Saat form berisi lokasi manual atau Share Location masuk, bot langsung mengirim update `Sebentar ya, aku cek dulu lokasi kamu…` sebelum AI/geocoding selesai. Koordinat Share Location langsung memenuhi kebutuhan lokasi. Alamat manual tetap dicoba melalui geocoder; jika tidak ditemukan, kesiapan dibatasi di bawah 70, laporan ditandai `needs_verification`, dan bot meminta koreksi ejaan, patokan terdekat, atau Share Location. Laporan tanpa titik peta tetap muncul di antrean verifikasi responder. Form juga meminta jenis bantuan serta konfirmasi apakah pelapor merupakan petani/penggarap di wilayah terdampak. AI menormalkan kebutuhan darurat dan pemulihan pertanian ke kategori baku agar dapat ditampilkan per laporan dan dijumlahkan per wilayah. Kebutuhan yang belum jelas hanya ditanyakan ulang satu kali; jawaban `BELUM TAHU` tidak menghambat laporan. Jawaban profil disimpan per laporan dan di profil; hanya pelapor lokal terkonfirmasi yang menjadi penerima alert berbasis radius.

Setiap laporan menyimpan confidence ekstraksi dan status verifikasi per field. Foto dinilai oleh model vision untuk relevansi terhadap dampak pertanian dan konsistensi dengan deskripsi; foto yang tidak relevan, tidak cocok, atau hanya diperiksa fallback heuristik tidak dihitung sebagai bukti terverifikasi pada alur WhatsApp produksi. Pemeriksaan ini adalah konsistensi visual, bukan autentikasi waktu/lokasi foto. Pastikan `OPENROUTER_MODEL` atau salah satu fallback mendukung input gambar. Dashboard responder menampilkan confidence, alasan keraguan, temuan visual, dan jumlah foto terverifikasi.

Kategori bantuan baku: evakuasi; bantuan medis; pangan; air bersih & sanitasi; tempat pengungsian; sandang & perlengkapan dasar; pompa & drainase; benih/bibit; pupuk & pestisida; alat/mesin pertanian; pakan & kesehatan ternak; dan perbaikan lahan/irigasi. Pelapor dapat menulis sinonim sehari-hari seperti `sembako`, `air minum`, atau `traktor`; classifier akan menggabungkannya ke kategori yang sesuai.
